...
/* eslint-disable no-magic-numbers, max-statements, prefer-template, global-require */

const check = require("./check");
const KEEP_IT = require("./symbols").KEEP_IT;
...
class ElectrodeKeepAlive {
  constructor(opts, hasScheduling) {
    hasScheduling =
      hasScheduling === undefined ? check.HAS_SCHEDULING : hasScheduling;

    if (hasScheduling) {
      const SchedulingAgent = require("./scheduling-agent");
      opts = this._makeOpts(
        { freeSocketTimeout: freeSocketKeepAliveTimeout, scheduling: "lifo" },
        opts
      );
      if (opts.hasOwnProperty("freeSocketKeepAliveTimeout")) {
        opts.freeSocketTimeout = opts.freeSocketKeepAliveTimeout;
        delete opts.freeSocketKeepAliveTimeout;
      }
      ...
      opts = this._makeOpts({ freeSocketKeepAliveTimeout }, opts);
  ...
  _makeOpts(defaultOpts, userOpts) {
    ...
    this._expiry = opts.expiry || FIVE_SECONDS_IN_MS;
  ...
  preLookup(host, options, cb) {
    ...
    dns.lookup(host, dnsOpts, (err, ip, addressType) => {
      ...
      const entry = {
        expiry: Date.now() + this._expiry + 50,
        ip,
        host,
        addressType
      };
